...
def main():
	...
	while _flag:
		global data
		global body
		body = read_port('../poi/cache/START2.json')["api_data"]
		data = read_port('../poi/cache/PORT.json')["api_data"]
...
def is_handled_by_predefined_func(input_cmd):
	...
	#event spring 2016
		...
		subprocess.call(['./E2.sh'], shell=True)
		print colored("伊401 : ふふーん♪", "green") + colored(" E2 ", "yellow") + colored("伊400型の追撃はしつこいんだから！", "green")
		return True
	elif input_cmd == 'ee3':
		...
		subprocess.call(['./1-5-a.sh'], shell=True)
		print colored("伊401", "green") + colored(" 1-5-a ", "yellow") + colored("出撃します！！", "green")
		return True
	elif input_cmd == '16':
		subprocess.call(['./1-6.sh'], shell=True)
		print colored("伊401", "green") + colored(" 1-6 ", "yellow") + colored("出撃します！！", "green")
		return True
	elif input_cmd == '22':
...
def e_task():
	...
	data = read_port(file_path)["api_data"]
